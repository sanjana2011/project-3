...
@WebServlet(urlPatterns = { "/ctl/TransListCtl" })

public class TransListCtl  extends BaseCtl{
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op) || "Previous".equalsIgnoreCase(op)) {
				...
				if (ids != null && ids.length > 0) {
					TransDTO deletedto = new TransDTO();
					for (String id : ids) {
			...
			dto = (TransDTO) populateDTO(request);
	...
	protected String getView() {
		...
		return ORSView.TRANS_LIST_VIEW;
	}
