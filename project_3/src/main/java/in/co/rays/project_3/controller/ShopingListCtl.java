...
@WebServlet(name = "ShopingListCtl", urlPatterns = { "/ctl/ShopingListCtl" })
public class ShopingListCtl extends BaseCtl {
...
//		dto.setId( DataUtility.getLong(request.getParameter("category")));
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op) || "Previous".equalsIgnoreCase(op)) {
				...
				if (ids != null && ids.length > 0) {
					ShopingDTO deletedto = new ShopingDTO();
					for (String id : ids) {
			...
			dto = (ShopingDTO) populateDTO(request);
	...
	protected String getView() {
		return ORSView.SHOPING_LIST_VIEW;
	}
